...
/* Reduce limits to fit within PSP RAM constraints */
#define RC_MAX_CONDITIONS    24
#define RC_MAX_GROUPS        4
#define RC_MAX_DELTA_SLOTS   128

/* Maximum achievements that can fire in a single evaluation pass.
 * In practice 2-3 is the most that will ever happen simultaneously,
 * but we keep a safe margin. */
...
typedef enum { RC_COND_STANDARD = 0, RC_COND_RESET_IF, RC_COND_PAUSE_IF, RC_COND_AND_NEXT, RC_COND_OR_NEXT, RC_COND_TRIGGER, RC_COND_MEASURED, RC_COND_MEASURED_IF, RC_COND_ADD_SOURCE, RC_COND_ADD_ADDRESS, RC_COND_SUB_SOURCE, RC_COND_RESET_NEXT_IF } RC_CondType;
...
/* Result now holds ALL achievements unlocked in one pass, not just one.
 * unlocked_count == 0 means nothing fired this frame. */
typedef struct {
    int                  unlocked_count;
